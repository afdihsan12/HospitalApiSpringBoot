...
@Controller
@CrossOrigin("*")
public class MedicinesControlloer {

	@Autowired
	private IMedicine roomstypeRepo;

	@PostMapping("/medicines")
	public ResponseEntity<?> addRoomsType(@RequestBody Medicines roomstype) {
		try {
	...
	public ResponseEntity<?> deleteRoomsType(@PathVariable("id") Long id) {
		Optional<Medicines> rt = roomstypeRepo.findById(id);
		try {
	...
	public ResponseEntity<?> editRoomsType(@PathVariable("id") Long id,@RequestBody Medicines rooms) {
		Optional<Medicines> rt = roomstypeRepo.findById(id);
	...
	public @ResponseBody List<Medicines> getAll(){
		return roomstypeRepo.findAll();
	}
	
	@GetMapping("/medicines/{id}")
	public @ResponseBody Medicines get1(@PathVariable("id") long id){
